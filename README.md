# Cross-lingual POS Tagging on Basque: Zero-shot vs. In-language Training
...
Multilingual pretrained language models such as XLM-RoBERTa have shown strong zero-shot cross-lingual transfer, but it remains unclear how much of this reflects genuinely shared, language-agnostic representations versus surface-level similarity between related languages. This project investigates that question by evaluating cross-lingual POS tagging transfer from English to Basque, a language isolate with no attested genealogical relatives, making it a linguistically motivated stress test for cross-lingual transfer.

Three systems are compared under matched conditions:

- **System A (Zero-shot)** — XLM-RoBERTa-base fine-tuned only on English (`UD_English-EWT`), evaluated directly on Basque with no exposure to labeled Basque data during training.
- **System B (In-language)** — XLM-RoBERTa-base fine-tuned directly on Basque (`UD_Basque-BDT`).
- **Frequency-based Baseline** — A non-neural baseline that tags each word with its most frequent UPOS tag observed in training, evaluated under both zero-shot and in-language conditions.

The better-performing system (System B) is then used to annotate the held-out Basque development split, producing a newly labeled corpus.
...
├── new_annotation.py          # Uses the trained System B model to annotate the Basque dev split
...
- [UD_Basque-BDT](https://github.com/UniversalDependencies/UD_Basque-BDT) — train/test splits used for System B; dev split used for the final annotation task
...
> Update the model checkpoint path in `new_annotation.py` to point to your saved System B checkpoint before running.
...
The output file `eu_bdt-ud-dev.system_b_annotated.conllu` contains the `UD_Basque-BDT` development split with UPOS tags predicted by System B, in the original CoNLL-U format with all other annotation layers (e.g., dependency relations) preserved unchanged.

## Model and Libraries

- **XLM-RoBERTa-base** ([Conneau et al., 2020](https://aclanthology.org/2020.acl-main.747/)) via HuggingFace `transformers`
